...
PROMPT;
	...
	private function validateAndApplyShowIf(array $questions): array {
		...
		foreach ($questions as $i => &$q) {
			...
			if (!is_string($refId) || !is_string($op)) {
				continue;
			}
			...
			if (in_array($parentType, ['number', 'scale', 'rating'], true) && in_array($op, $stringOnlyOps, true)) {
				continue;
			}
			if (in_array($parentType, ['choice', 'multiple', 'dropdown'], true) && in_array($op, $numericOnlyOps, true)) {
				continue;
			}
			...
			if (in_array($parentType, ['choice', 'multiple', 'dropdown'], true) && in_array($op, ['equals', 'notEquals', 'in', 'notIn'], true)) {
				...
				if ($matched === null) {
					continue;
				} // drop — value doesn't match any option
				$value = $matched;
			}
