...
export default {
  ...
  setup(props, { emit }) {
    ...
    // add an email question to the form (and mark it useAsRespondentEmail
    // so the backend knows which field to send to). When disabled we only
    // clear the flag — we don't delete the question so any responses that
    // already used it remain meaningful (#103).
    const toggleConfirmationEmail = async (enabled) => {
      responseSettings.sendConfirmationEmail = !!enabled;
      try {
        const questions = Array.isArray(props.form.questions) ? [...props.form.questions] : [];
        const existing = questions.find(q => q.useAsRespondentEmail);
        ...
        if (enabled) {
          if (!existing) {
            ...
            if (pages) {
              pages[pages.length - 1].questions.push(newQuestion.id);
            }
          } else {
            existing.useAsRespondentEmail = true;
          }
        } else if (existing) {
          // Remove the auto-generated email question on disable so the
          // form is back to its original shape. If the user customised it
          // (cleared the autoGenerated flag), we leave it alone and only
          // strip the respondent-email flag.
          if (existing.autoGenerated) {
            const idx = questions.indexOf(existing);
            if (idx !== -1) questions.splice(idx, 1);
            // Also drop its id from whichever page holds it (#6).
            if (pages) {
              for (const p of pages) {
                const qi = p.questions.indexOf(existing.id);
                if (qi !== -1) p.questions.splice(qi, 1);
              }
            }
          } else {
            existing.useAsRespondentEmail = false;
          }
        }
