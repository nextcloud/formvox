      ...
      <div class="editor-container">
        ...
        <div class="editor-header">
          ...
          <div class="form-field">
            ...
            <MarkdownEditor
              ...
              :align="form.descriptionAlign || 'left'"
              :disabled="!canEdit"
              :placeholder="t('Enter description (optional)')"
              class="description-input"
              @update:model-value="debouncedSave"
              @update:align="updateDescriptionAlign"
            />
          </div>
        </div>
        ...
        <div v-else class="questions-container">
          ...
          <div v-if="canEdit && !showPreview" class="add-question-rail">
            ...
            <NcButton type="tertiary" @click="addDescriptor">
              <template #icon>
                <InfoIcon :size="20" />
              </template>
              {{ t('Add info block') }}
  ...
  setup(props) {
    ...
    const currentPageQuestions = computed({
      get() {
        ...
        if (!page) return [];
        // Return the actual question objects for this page, hiding
        // auto-generated helper questions (e.g. confirmation email) #103.
        return page.questions
          .map(qId => form.questions.find(q => q.id === qId))
          .filter(q => q && !q.autoGenerated);
      },
      set(newQuestions) {
        if (!hasPages.value) return;
        // Update the page's question IDs based on the new order, preserving
        // any auto-generated helper questions at their original position.
        const visibleIds = newQuestions.map(q => q.id);
        const autoIds = form.pages[currentPageIndex.value].questions
          .filter(qId => {
            const q = form.questions.find(x => x.id === qId);
            return q && q.autoGenerated;
          });
        form.pages[currentPageIndex.value].questions = [...visibleIds, ...autoIds];
        debouncedSave();
      }
    });

    // Single-page editor view: visible (non-auto-generated) questions only.
    const visibleFormQuestions = computed({
      get() {
        return (form.questions || []).filter(q => !q.autoGenerated);
      },
      set(newQuestions) {
        // Merge: keep auto-generated questions at the end of the array.
        const auto = (form.questions || []).filter(q => q.autoGenerated);
        form.questions = [...newQuestions, ...auto];
        debouncedSave();
      }
    });
      ...
      try {
        await axios.put(
          generateUrl('/apps/formvox/api/form/{fileId}', { fileId: props.fileId }),
          {
            ...
            descriptionAlign: form.descriptionAlign || 'left',
            questions: form.questions,
            settings: form.settings,
            pages: form.pages,
            branding: form.branding,
    ...
    const updateDescriptionAlign = (align) => {
      form.descriptionAlign = align;
      debouncedSave();
    };
    ...
    const addDescriptor = () => {
      // A descriptor is a markdown info block — it renders content to the
      // respondent but has no answer, so it's excluded from validation,
      // results, and CSV/JSON exports just like a section header (#64).
      const newDescriptor = {
        id: `d${uuidv4().split('-')[0]}`,
        type: 'descriptor',
        question: '',
      ...
      form.questions.push(newDescriptor);
      if (hasPages.value && form.pages[currentPageIndex.value]) {
        form.pages[currentPageIndex.value].questions.push(newDescriptor.id);
      }
      debouncedSave();
    };
